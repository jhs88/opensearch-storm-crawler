...
``` sh
storm local target/crawler-1.0.jar --local-ttl 60 com.digitalpebble.CrawlTopology -- -conf crawler-conf.yaml
```
...
``` sh
storm local target/crawler-1.0.jar  org.apache.storm.flux.Flux crawler.flux --local-ttl 3600
```
